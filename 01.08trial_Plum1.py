#Date: 01/08/24
#Author: Jacky 
#Purpose: Colour Trial 3 - Plum1 and Yellow
...
def main():
    ...
    main_window.configure(bg='Plum1')
...
def print_hired_item_details():
    ...
    Label(main_window, font=("Segoe UI", 10, "bold"), text="Row #", bg="Yellow").grid(column=0, row=10, padx=1, pady=5)  # Row Heading
    Label(main_window, font=("Segoe UI", 10, "bold"), text="Receipt Number:", bg="Yellow").grid(column=1, row=10, padx=1, pady=5) # Reciept Number Heading
    Label(main_window, font=("Segoe UI", 10, "bold"), text="Name:", bg="Yellow").grid(column=2, row=10, padx=1, pady=5) # Name Heading
    Label(main_window, font=("Segoe UI", 10, "bold"), text="Item Name:", bg="Yellow").grid(column=3, row=10, padx=1, pady=5) # Item Name Heading
    Label(main_window, font=("Segoe UI", 10, "bold"), text="Quantity Hired:", bg="Yellow").grid(column=4, row=10, padx=1, pady=5) # Quantity Hired Heading

    # Add each item in the list into its own row
    while name_count < counters['total_entries']:
        Label(main_window, text=str(name_count + 1), fg="black", bg="Plum1").grid(column=0, row=name_count+11) # Row number
        Label(main_window, text=hired_items[name_count][3], fg="black", bg="Plum1").grid(column=1, row=name_count+11) # Receipt Number
        Label(main_window, text=hired_items[name_count][0], fg="black", bg="Plum1").grid(column=2, row=name_count+11) # Name
        Label(main_window, text=hired_items[name_count][1], fg="black", bg="Plum1").grid(column=3, row=name_count+11) # Item Name
        Label(main_window, text=hired_items[name_count][2], fg="black", bg="Plum1").grid(column=4, row=name_count+11) # Quantity Hired
        name_count += 1
    ...
    try:
        ...
        if row_number < 0 or row_number >= counters['total_entries']:
            ...
            for widget in main_window.winfo_children():
                if isinstance(widget, Label) and widget.cget('text') != "" and widget.cget('bg') == 'Plum1':
                    widget.destroy()
...
def setup_buttons():
    ...
    Label(main_window, font=("Segoe UI", 10), text="Customer Full Name", bg='Plum1').grid(column=0, row=1, sticky=E)
    Label(main_window, font=("Segoe UI", 10), text="Item Name", bg='Plum1').grid(column=0, row=2, sticky=E)
    Label(main_window, font=("Segoe UI", 10), text="Quantity Hired", bg='Plum1').grid(column=0, row=3, sticky=E)
    Button(main_window, font=("Segoe UI", 10), text="Quit", bg='Plum1', command=quit, width=10).grid(column=4, row=1, sticky=E)
    Button(main_window, font=("Segoe UI", 10), text="Submit", bg='Plum1', command=check_inputs).grid(column=3, row=2, sticky=E)
    Button(main_window, font=("Segoe UI", 10), text="Print Details", bg='Plum1', command=print_hired_item_details, width=10).grid(column=4, row=2, sticky=E)
    Label(main_window, font=("Segoe UI", 10), text="Row #", bg='Plum1').grid(column=3, row=3, sticky=E)
    Button(main_window, font=("Segoe UI", 10), text="Delete Row", bg='Plum1', command=delete_row, width=10).grid(column=4, row=4, sticky=E)
   ...
   # Load and place the bin icon next to the "Delete Row" button
    try:
        ...
        bin_icon_label = Label(main_window, image=bin_icon, bg='Plum1')

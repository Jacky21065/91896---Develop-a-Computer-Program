...
def validate_name(input_value):
    if len(input_value) > 25:
        messagebox.showerror('Invalid Input', 'Customer Full Name: Cannot exceed 15 characters.')
        return False
...
def open_receipt_window():
    ...
    if hired_items:
        ...
        for i, item in enumerate(hired_items):
            ...
            Label(rceipt_window, font=("Segoe UI", 14, "bold"), text=item[3], fg="black", bg="#3CB371", anchor='center').grid(column=1, row=i + 1, **heading_padding, sticky='ew')
...
def check_inputs():
    ...
    # Check the number of Quantity Hired is not below 1, otherwise Error Display Message
        ...
        if quantity < 1:
            messagebox.showerror('Invalid Input', 'Quantity Hired: Value must be higher and in between 1 - 500.')
            input_check = 1
    # Check the number of Quantity Hired is not above 500, otherwise Display Error Message
        elif quantity > 500:
            messagebox.showerror('Invalid Input', 'Quantity Hired: Value must be lower and in between 1 - 500.')
            input_check = 1
    else:
...
def setup_buttons():
    ...
    # Initialize Spinbox with no value (empty)
    entry_quantity.delete(0, 'end')

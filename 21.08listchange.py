...
def validate_name(input_value):
    ...
    """
    if len(input_value) <= 25 and all(char.isalpha() or char.isspace() for char in input_value):
        return True
    elif input_value == "":
        return True  # Allow empty spaces in between names
    if len(input_value) > 25:
        messagebox.showerror(
            'Invalid Input', 'Customer Full Name: Please reduce your entry length. \nYour entry cannot exceed 25 characters, including spaces.'
        )
        return False
    elif any(char.isdigit() for char in input_value):
        messagebox.showerror(
            'Invalid Input', 'Customer Full Name: Please input only valid alphabet letters and spaces, not numeric values.'
        )
        return False
    elif not all(char.isalpha() or char.isspace() for char in input_value):
        messagebox.showerror(
            'Invalid Input', 'Customer Full Name: Please input only valid alphabet letters and spaces, not symbols.'
        )
        return False
    else:
        messagebox.showerror(
            'Invalid Input', 'Customer Full Name: Please input only valid alphabet letters and spaces.'
        )
        return False
...
def validate_entry_delete_row(input_value):
    """
...
def main():
    ...
    icon_image = PhotoImage(file=os.getcwd()+r"\images\icon.png") 
...
def check_inputs():
    ...
    full_name = entry_full_name.get().title()
    ...
    if len(item_name_combo_box.get()) == 0:
...
def append_item(full_name):
    ...
    hired_items.append([full_name, item_name_combo_box.get(), entry_quantity_hired.get(), receipt_number])
...
def delete_row():
    try:
        ...
        # int(entry_delete_row.get()) converts the inputted entry from string type to integer type.
        ...
        row_number = int(entry_delete_row.get()) - 1
  ...
  # Instead of trying to delete an invalid row, show an error message to the user indicating that the row number is invalid.
        ...
        messagebox.showerror('Error', 'Invalid Row Number. Please input only valid row numbers. See the "Receipts" window for valid row numbers.')
...
def setup_buttons():
    ...
    # Setup the entry fields: entry_full_name, item_name_combo_box, entry_quantity_hired and entry_delete_row.
    global entry_full_name, item_name_combo_box, entry_quantity_hired, entry_delete_row
    entry_full_name = Entry(main_window, width=24, validate='key', validatecommand=(main_window.register(validate_name), '%P'))
    entry_full_name.grid(column=1, row=1, padx=1, pady=10, sticky=W)

    item_name_combo_box = ttk.Combobox(main_window, width=21, state='readonly', validate='key', validatecommand=(main_window.register(validate_name), '%P'))
    item_name_combo_box['values'] = ('Bamboo Forks', 'Bamboo Spoons', 'Bamboo Plates', 'Bamboo Cups', 'Paper Hats', 'Biodegradable Balloons')
    item_name_combo_box.grid(column=1, row=2, padx=1, pady=10, sticky=W)

    entry_quantity_hired = Entry(main_window, width=24, validate='key', validatecommand=(main_window.register(validate_quantity), '%P'))
    entry_quantity_hired.grid(column=1, row=3, padx=1, pady=10, sticky=W)

    entry_delete_row = Entry(main_window, width=24, validate='key', validatecommand=(main_window.register(validate_entry_delete_row), '%P'))
    entry_delete_row.grid(column=3, row=6, padx=1, pady=10, sticky=W)    

    # Create a frame for the help and terms buttons at the bottom center.
    button_frame = Frame(main_window, bg='MediumSpringGreen')
    button_frame.grid(column=0, row=10, columnspan=5, pady=20, sticky='ew')

    # Add Help and Terms buttons to the bottom frame.
    Button(button_frame, font=("Segoe UI", 16, "bold"), text="Help", bg='orange', bd=5, relief=RAISED, command=open_help_window).pack(side=LEFT, padx=10)
    Button(button_frame, font=("Segoe UI", 16, "bold"), text="Terms", bg='orange', bd=5, relief=RAISED, command=open_terms_window).pack(side=LEFT, padx=10)

   # Set the weight of the row containing the button_frame to push it down.
...
# Define function on the "Terms and Conditions" window that will open when the user clicks on the "Terms" button.

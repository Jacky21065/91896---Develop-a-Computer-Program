...
def setup_buttons():
    ...
    Button(main_window, font=("Segoe UI", 16, "bold"), text="Delete Row", bg='orange', bd=5, image = new_player_background_image,compound=RIGHT, relief=RAISED, command=delete_row).grid(column=4, row=6, sticky=E, padx=5, pady=5)
    ...
    new_player_background_image=PhotoImage(file=r"\images\rubbish_bin.png").convert("RGBA")
    width, height = new_player_background_image.size
    new_player_background_image=new_player_background_image.resize((width // 10, height // 10), Image.LANCZOS)
    new_player_background=ImageTk.PhotoImage(new_player_background_image)

    terms_background_image=Image.open(os.getcwd()+r"\images\terms_sign.png").convert("RGBA")
    width, height = terms_background_image.size
    terms_background_image=terms_background_image.resize((width // 10, height // 10), Image.LANCZOS)
    terms_background=ImageTk.PhotoImage(terms_background_image)

    label = Label(main_window, image=new_player_background)
    label.place(x=700,y=100)
    label.image=new_player_background

    label = Label(main_window, image=terms_background_image)
    label.place(x=50,y=100)
    label.image=terms_background_image

...
def print_hired_item_details():
    ...
    Label(main_window, font=("Segoe UI", 10, "bold"), text="Receipt Number:", bg='red').grid(column=0, row=7)
    Label(main_window, font=("Segoe UI", 10, "bold"), text="Name:", bg='red').grid(column=1, row=7)
    Label(main_window, font=("Segoe UI", 10, "bold"), text="Quantity Hired:", bg='red').grid(column=2, row=7)
    Label(main_window, font=("Segoe UI", 10, "bold"), text="Item Name:", bg='red').grid(column=3, row=7)
...
def check_inputs():
    input_check = 0
    Label(main_window, text="                       ", bg="cyan").grid(column=2, row=0)
    Label(main_window, text="                     ", bg="cyan").grid(column=2, row=1)
    Label(main_window, text="                        ", bg="cyan").grid(column=2, row=2)
    Label(main_window, text="                     ", bg="cyan").grid(column=2, row=3)
    ...
    if len(entry_name.get()) == 0 or not entry_name.get().isalpha():
        Label(main_window, bg='cyan', fg='red', text="Required / Only letters allowed").grid(column=2, row=1)
        input_check = 1
    # Check that item is selected, set error text if blank
    if len(combo_box.get()) == 0:
        Label(main_window, bg='cyan', fg='red', text="Required").grid(column=2, row=2)
        input_check = 1
    ...
    if entry_quantity.get().isdigit():
        ...
        if quantity < 1 or quantity > 500:
            Label(main_window, bg='cyan', fg='red', text="1-500 only").grid(column=2, row=3)
            input_check = 1
    else:
        Label(main_window, bg='cyan', fg='red', text="Required").grid(column=2, row=3)
        input_check = 1 
...
def setup_buttons():
    ...
    global combo_box
    combo_box = ttk.Combobox(main_window, width=20, state='readonly')  # Set width and make it read-only
    combo_box['values'] = ('Plates', 'Cups', 'Spoons', 'Fork')  # Define your item options

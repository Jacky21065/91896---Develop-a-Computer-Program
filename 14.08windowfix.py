...
def open_reciept_window():
    reciept_window = Toplevel(main_window)
    reciept_window.title("Reciepts")
    reciept_window.geometry("1200x600")
    reciept_window.resizable(False,False)
    reciept_window.configure(bg='MediumSpringGreen')

    # Create column headings in the Receipt window
    Label(reciept_window, font=("Segoe UI", 18, "bold"), text="Row #:", bg="DeepSkyBlue2").grid(column=0, row=0, padx=1, pady=5)
    Label(reciept_window, font=("Segoe UI", 18, "bold"), text="Receipt #:", bg="DeepSkyBlue2").grid(column=1, row=0, padx=1, pady=5)
    Label(reciept_window, font=("Segoe UI", 18, "bold"), text="Customer Full Name:", bg="DeepSkyBlue2").grid(column=2, row=0, padx=1, pady=5)
    Label(reciept_window, font=("Segoe UI", 18, "bold"), text="Item Name:", bg="DeepSkyBlue2").grid(column=3, row=0, padx=1, pady=5)
    Label(reciept_window, font=("Segoe UI", 18, "bold"), text="Quantity Hired:", bg="DeepSkyBlue2").grid(column=4, row=0, padx=1, pady=5)

    # Display each item in the Receipt window
    for i, item in enumerate(hired_items):
        Label(reciept_window, text=str(i + 1), fg="black", bg="MediumSpringGreen").grid(column=0, row=i+1)
        Label(reciept_window, text=item[3], fg="black", bg="MediumSpringGreen").grid(column=1, row=i+1)
        Label(reciept_window, text=item[0], fg="black", bg="MediumSpringGreen").grid(column=2, row=i+1)
        Label(reciept_window, text=item[1], fg="black", bg="MediumSpringGreen").grid(column=3, row=i+1)
        Label(reciept_window, text=item[2], fg="black", bg="MediumSpringGreen").grid(column=4, row=i+1)

    # If no data to display
    if not hired_items:
        Label(reciept_window, text="No data to display.\nNeed Help? Use the Help button on the main window.", font=("Segoe UI", 20, "bold"), bg='red').grid(column=1, row=5, padx=5, pady=5, columnspan=5)
...
def append_item():
    ...
    save_info()  
    # Open the Receipt window to show updated data
    open_reciept_window()
...
def setup_buttons():
    ...
    Button(main_window, font=("Segoe UI", 16, "bold"), text="Receipts", bg='orange', bd=5, relief=RAISED, command=open_reciept_window).grid(column=4, row=2, sticky=E, padx=5, pady=5)

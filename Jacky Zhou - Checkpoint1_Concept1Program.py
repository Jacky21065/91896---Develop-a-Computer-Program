#Date: 18/06/2024
#Author: Jacky
#Purpose: Checkpoint 1 AS91896 - Concept 1 Program FINAL
...
def print_hired_item_details():
    ...
    Label(main_window, font=("Segoe UI", 10, "bold"), text="Receipt #:").grid(column=0, row=7)
...
def append_item():
    ...
    hired_items.append([entry_name.get(), entry_item.get(), entry_quantity.get()])
    # Clear the boxes
...
def setup_buttons():
    ...
    Label(main_window, text="Item Name").grid(column=0, row=1, sticky=E)
    Label(main_window, text="Items Hired").grid(column=0, row=2, sticky=E)
